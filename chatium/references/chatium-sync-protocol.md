...
- `finish` records task changes relative to the stored Chatium baseline before `pull`, creates a fresh baseline from the latest server state, reapplies the stash, uploads the recorded task changes, and leaves any still-uncommitted local edits uncommitted for user review. Changes that were already committed locally after `begin` stay committed locally, but are still uploaded. Dirty files that already match the Chatium `syncedChecksum` are skipped instead of uploaded again. It uses the same baseline-preserving merge for already-synced dirty git files.
